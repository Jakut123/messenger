...
class ForgotPasswordCompleteSerializer(serializers.Serializer):
    email = serializers.EmailField(required=True)
    new_code = serializers.CharField(required=True, min_length=8, max_length=8)
    ...
    def validate_forgotpassword(self, new_code):
        if not User.objects.filter(activation=new_code).exists():
            raise serializers.ValidationError('Пользователь не зарегистрирован')
        return new_code
